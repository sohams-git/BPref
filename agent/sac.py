...
class SACAgent(Agent):
    ...
    def __init__(
        ...
        # Save configs and lrs for resets
        self.critic_cfg = critic_cfg
        self.critic_lr = critic_lr
        self.critic_betas = critic_betas
        ...
        # --------- Build networks WITHOUT hydra.instantiate ---------
    ...
    # ------------------------------------------------------------------
    # Helper builders: read class path from Hydra config and instantiate
    # ------------------------------------------------------------------
    def _build_critic(self):
        """
        Build a critic from self.critic_cfg:
          critic_cfg["class"]  -> "agent.critic.DoubleQCritic"
          critic_cfg["params"] -> dict of kwargs
        """
        cfg = self.critic_cfg
        class_path = cfg["class"]
        module_name, cls_name = class_path.rsplit(".", 1)
        module = importlib.import_module(module_name)
        CriticCls = getattr(module, cls_name)
        params = dict(cfg["params"])
        return CriticCls(**params).to(self.device)

    def _build_actor(self):
        """
        Build an actor from self.actor_cfg:
          actor_cfg["class"]  -> "agent.actor.DiagGaussianActor"
          actor_cfg["params"] -> dict of kwargs
        """
        cfg = self.actor_cfg
        class_path = cfg["class"]
        module_name, cls_name = class_path.rsplit(".", 1)
        module = importlib.import_module(module_name)
        ActorCls = getattr(module, cls_name)
        params = dict(cfg["params"])
        return ActorCls(**params).to(self.device)

    # ------------------------------------------------------------------
    # Reset methods now use the same helpers (no hydra.instantiate)
    # ------------------------------------------------------------------
    def reset_critic(self):
        self.critic = self._build_critic()
        self.critic_target = self._build_critic()
    ...
    # ------------------------------------------------------------------
    # Rest of the original SACAgent code unchanged
    # ------------------------------------------------------------------
    def train(self, training=True):
        self.training = training
        self.actor.train(training)
